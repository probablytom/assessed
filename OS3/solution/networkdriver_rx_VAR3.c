...
void *receiver_fn (void *args) {
    ...
    blocking_get_pd(myargs->fpds, NULL);
    init_packet_descriptor(NULL);
    register_receiving_packetdescriptor(myargs->nd,NULL);
    for(;;) {
    	// Wait for incoming packets

        filled_pd = NULL;
	if (nb_get_pd(myargs->receiver_pool, myargs->fpds, NULL)) {
